"""
AG Vision - V8 Tracked Pipeline (Multi-Person Optimised)
Fix: Resolves age/gender cross-contamination between multiple people.
Key change: YOLOv8 .track(persist=True) + TrackedViTEngine (per-ID queues)
...
def draw_hud(frame, fps, modules, active_tracks):
    ...
    cv2.putText(frame, f"ANTIGRAVITY V8 TRACKED - FPS: {int(fps)}",
...
def main():
    print("==================================================")
    print("  🚀 ANTIGRAVITY V8 - TRACKED MULTI-PERSON 🚀")
    print("==================================================")
    print("  FIX: Each person gets independent age/gender predictions.")
    ...
    while True:
            ...
            for i, box in enumerate(boxes):
                ...
                if modules["p"][1] and not is_known:
                    ...
                    cv2.putText(display, label, (x1 + 5, y1 - 15),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.50, box_color, 2)
        ...
        cv2.imshow("Antigravity V8 TRACKED", display)
