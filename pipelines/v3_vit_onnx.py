...
def main():
    print("[*] Starting Pipeline V3: YOLOv8 + Async ViT/ONNX...")
    load_config()
    ...
    while True:
        ...
        cv2.putText(frame, f"V3 ViT Async - FPS: {int(fps)}", (10, 30),
        ...
        if len(results) > 0 and len(results[0].boxes) > 0:
            for box in results[0].boxes:
                ...
                if (time.time() - last_submit) > 0.4:
                    async_vit.submit(face_crop)
                    last_submit = time.time()

                res = async_vit.get_latest()
                s_age = str(res["age"]) if res["age"] != "--" else "..."
                s_gen = res["gender"]
